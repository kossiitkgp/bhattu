...
def handle_command(command, channel, msg, usernm):
        ...
        while(1):
            ...
            if pos > 0:
                ...
                response = response[0:pos] + "`" + \
                    "@bhattu" + "`" + response[pos2 + 1:]
            else:
                break
    else:
        response = "Please start your message with `@bhattu`. Use `@bhattu /help` for help."
    ...
    if (chs == []):
        ...
        for i in chs:
            slack_client.api_call("chat.postMessage", channel=(
                str(i))[1:str(i).find('|')], text=response, as_user=True)
...
try:
    ...
    def send_message(user):
        ...
        slack_client.api_call(
            ...
            channel="#random",
            as_user=True,
            text="{} How about we have chai and sutta at Chedis ..? :stuck_out_tongue_winking_eye:".format(
                user)
        )
    ...
    if __name__ == "__main__":
        ...
        if slack_client.rtm_connect():
            ...
            while True:
                # Output format 2009-01-05 22:14:39
                timenow = str(strftime("%Y-%m-%d %H:%M:%S", gmtime()))
                if timenow[11:13] == '21' and timenow[14:16] == '30' and count == 0:
                    users = slack_client.api_call("users.list", channel=os.environ[
                                                  "CH_RANDOM_ID"])["members"]
                    onlineUsers = ""
                    totalUsers = 0
    ...
    if channel != "":
        slack_client.api_call("chat.postMessage", channel=channel,
                              text="Oops! Something went wrong :disappointed:...", as_user=True)
